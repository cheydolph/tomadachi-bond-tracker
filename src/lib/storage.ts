import { ALL_BOND_LEVELS, DATA_VERSION, STORAGE_KEY } from './types'
import type { BondData, BondLevel } from './types'
...
export function loadData(): BondData {
  ...
  if (corrections > 0) {
    console.info(
      `[TBT] Migrated ${corrections} asymmetric bond pair(s) to symmetric.`,
    )
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const { data: migrated, corrections } =
          migrateToSymmetricBonds(validated)
        if (corrections > 0) {
          console.info(
